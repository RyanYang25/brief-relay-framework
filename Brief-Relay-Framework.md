...
- **Gate logic** (BRF specifies *what to check* and *what to output*, not the language): parse the four-state markers in `current.md` and judge item-by-item:
  ...
  | **Process-flexibility self-check (v1.8)** | Do in-flight tasks declare takeover intent and size tier? Do heavy tasks declare a standards alignment check? (see 18.4) | Light/Medium may skip; Heavy must declare explicitly (WARN only, non-blocking) |
...
- **Minimal reference implementation (v1.6, enhanced in v1.7/v1.8)**: the BRF project space itself ships `tools/four-state-gate.ps1` (PowerShell, plain file + cross-platform readable) — it parses `current.md`'s four-state markers + compares contract-header `last_updated` + checks file-encoding health, and emits a "readiness report"; verified on 2026-08-31; **v1.7 adds the document-health smell family (see 17.5: dangling links / leftover placeholders / missing required fields / open-UNKNOWN count / active-file oversize, split ERROR/WARN), verified on 2026-09-01; v1.8 adds the process-flexibility self-check family (see 18.4: takeover-intent / size-tier markers + standards-alignment declaration, WARN only), verified the same day**. New projects may follow this implementation or choose their own language (Shell / Python / PowerShell all fine); BRF only dictates "what to check + what to output", not the language.
...
### XVIII. Process Flexibility: Tiered Takeover & Task Routing (new in v1.8)

> **Source**: a 2026-09-01 GitHub external benchmark — BMAD-METHOD's "right-sized process" (flow depth adapts to workload), repo-context-ledger's takeover-intent classifier ("understand / tweak / normal change / continue"), github/spec-kit's constitution (project constitution + per-phase alignment back-check). After v1.7 landed "Handoff Trustworthiness", the BRF project space distilled these three mechanisms into portable enhancements — solving "every task runs the same full pipeline, small tasks feel heavy". All pass the dual-track filter (domain-agnostic / no local-ops details / reusable by others).

v1.5→v1.7 made handoff *formally* machine-checkable and *content* trustworthy, but the recovery protocol was still a **single path**: every takeover ran the same full pipeline. v1.8 upgrades it to a **tiered path** — the taker first judges "what am I here for" (intent) and "how big is this job" (size), then decides how much to read and how deep to go; meanwhile standards is promoted from "a list of do-not-change rules" to "the project constitution", anchored by alignment checks so that elasticity never drifts out of control.

#### 18.1 Takeover-intent classification (decides how much to read)

Right after step one, the taker judges what this visit is for, then decides reading depth; when intent is ambiguous, clarify rather than guess (from repo-context-ledger's intent classifier).

| Intent | Meaning | Minimal reading set | Persistence requirement |
|--------|---------|--------------------|-------------------------|
| Understand | Read-only: clarify / report / explain / evaluate | BRIEF + current (mandatory) | Usually none |
| Tweak | One-point small edit: a value / a copy / a format | BRIEF + current + target file | Persist that change + one-line note |
| Change | Substantive change touching structure / scope / multiple files | BRIEF + current + standards + relevant files | Full incremental persistence + four-state record |
| Continue | Resume an in-flight task: restore context and carry on | BRIEF + current + last round's Open/UNKNOWN | Update Open/UNKNOWN/Decisions |

> Relationship with the 9.1 recovery protocol: the protocol is "the mandatory first step of any takeover" (read BRIEF + current, compare timestamps, reconcile); intent classification is "the second step" — it decides how deep this baton should read. Light intents may skip heavy reading, but the protocol's first step is never skipped.

#### 18.2 Task-size tiering (decides how deep to go)

Choose process depth by workload (from BMAD's right-sized process: small changes go straight to build; complex work gets deep planning).

| Tier | Applies to | Process depth | Mandatory actions |
|------|-----------|---------------|-------------------|
| Light (L) | Tweak-level, single point, low impact | Direct, little documentation | Persist change + one-line note |
| Medium (M) | Change-level, one file or a small multi-file scope | Incremental persistence + four states | Four-state closure + incremental persistence |
| Heavy (H) | Cross-stage, high impact, touching spec / structure / release | Full pipeline | Validation statement (17.1) + UNKNOWN ledger (17.2) + four-state closure + standards alignment (18.4) |

Principle: process depth follows **task size**, not "whether the project is important"; the same method covers weekend prototypes and long-lived systems. Tiering is a **labour-saving option**, not a loosening of discipline — Light/Medium "less documentation" means fewer formal docs, but changes must still be persisted and records must still leave a trace.

#### 18.3 Takeover-depth routing table (intent × size → must-read + may-skip)

| Scenario (intent × size) | Minimal must-read set | May skip | Mandatory wrap-up |
|--------------------------|-----------------------|----------|-------------------|
| Understand × L/M | BRIEF + current | standards / PROJECT_GUIDANCE / history | None (read-only) |
| Tweak × L | BRIEF + current + target file | standards / PROJECT_GUIDANCE / full four states | Persist + note |
| Tweak × M | BRIEF + current + standards + target file | PROJECT_GUIDANCE history | Persist + recheck Open/UNKNOWN |
| Change × M | BRIEF + current + standards + relevant files | PROJECT_GUIDANCE history | Four-state closure |
| Change × H | Full read (BRIEF + current + standards + PROJECT_GUIDANCE + references as needed) | None | Validation statement + UNKNOWN + standards alignment + four-state closure |
| Continue × M/H | BRIEF + current + last Open/UNKNOWN + relevant files | Unrelated history | Update Open/UNKNOWN/Decisions |

> The routing table is a "suggested default", not a hard rule: trimming is allowed when context windows are tight or the task boundary is clear, but **trims must be stated explicitly** (to prevent silently skipping key checks).

#### 18.4 standards as constitution & alignment check (the anchor that keeps elasticity from drifting)

- **standards is the project constitution**: it carries the project's immutable principles (naming / red lines / positioning / lessons learned), **append-only, never edited** (see Ch. VII update rules); no change may violate a constitutional clause.
- **Alignment check**: a Change × Heavy task must run a "standards back-check" at wrap-up — does this change touch any constitutional clause? If so, explicitly declare "aligned / how aligned"; if the constitution itself genuinely needs revision, follow the append-only flow (add a clause, never edit an old one).
- **Machine-checkable landing (v1.8)**: the gate adds a "process-flexibility self-check" — do in-flight tasks declare takeover intent and size tier (`intent=… · size=…`)? Does a heavy task's wrap-up declare a standards back-check? Both are WARN-only, non-blocking (Light/Medium may skip).
- Intent and size are **self-declared by the taker** (written into the current.md task row or a Decision record); the gate only reminds "you didn't declare it", it does not judge whether a task is Light or Heavy — tiering is a rule, not a classifier outsourced to a machine.

#### 18.5 Division of labour with existing chapters

| Chapter | Governs | Relationship to this chapter |
|---------|---------|-----------------------------|
| 9.1 Recovery protocol | Takeover step one (read BRIEF+current, compare timestamps, reconcile) | Not replaced; intent classification is step two |
| 15.1 Four-state patrol | Full self-check at iteration close-out | "Heavy" here forces the full four states; Light/Medium may trim but must not break four-state closure |
| 17.1 / 17.2 | Validation statement / UNKNOWN ledger | Mandatory for "Heavy"; Light/Medium may lighten as needed |
| 17.4 Capacity threshold | Active-file volume | With tiering lightening the load, Light/Medium tasks write less and naturally control volume |

#### 18.6 Deliberately excluded: tiering is a rule, not a platform service
